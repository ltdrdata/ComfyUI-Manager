...
def get_pip_cmd():
    # Check if .venv/bin/uv exists, indicating uv is being used
...
Execute following commands:
    {' '.join(pip_cmd)} uninstall ultralytics
    {' '.join(pip_cmd)} install ultralytics==8.3.40
...
https://blog.comfy.org/comfyui-statement-on-the-ultralytics-crypto-miner-situation/
    ...
    installed_pips = subprocess.check_output(pip_cmd + ["freeze"], text=True)

    detected = set()
    try:
        anthropic_info = subprocess.check_output(pip_cmd + ["show", "anthropic"], text=True, stderr=subprocess.DEVNULL)
